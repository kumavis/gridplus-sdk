...
export class Client {
   ...
   * @param params.baseUrl [[baseUrl]]
   * @param params.crypto - [[crypto]] 
   * @param params.name - [[name]] 
   * @param params.privKey - [[privKey]] 
   * @param params.key - [[key]] 
   * @param params.pairingSalt - [[pairingSalt]] 
   * @param params.retryCount - [[retryCount]] 
   * @param params.timeout - [[timeout]]
   */
   ...
   * @category Lattice
   * @param pairingSecret - The pairing secret that was generated by the device.
   ...
   * @category Lattice
   * @param data  
   * @param data.payload - TODO
   * @param data.testID - TODO
   * @param cb - callback function
   * @returns The decrypted data.
   */
  test (data: { payload: any, testID: number }, cb: (a, b?) => any) {
    if (!data.payload)
   ...
   * @category Lattice
   * @param opts - Options 
   * @param opts.startPath - TODO
   * @param opts.n - TODO
   * @param cb - callback function
   * @returns An array of addresses.
   */
  getAddresses (opts: { startPath: number[], n: UInt4 }, cb) {
  ...
  /**
   * @param {any} fwVersion - The version of the firmware that is being uploaded.
   * 
   */
   ...
   * @param opts.currency -
   * @param opts.data -
   * @param cb - The callback function to call when the request is complete.
   * @param cachedData - This is the cached data from the previous request. If this is null,
   * @param nextCode - the next code to send to the device. This is only used if there are
   * @returns The response from the device.
   */
  sign (opts: { currency, data }, cb, cachedData = null, nextCode = null) {
    const { currency } = opts;
    let { data } = opts;
   ...
   * @category Lattice
   * @param defs - An array of ABI definitions to add.
   * @param cb - The callback to call when the request is complete.
   * @param nextCode - If this is a follow-up request, we don't need to ask for user authorization
   ...
   * @param cb - The callback function to be called when the request is complete.
   * @returns The callback is called with an error if there is one, otherwise it is called with no
   * arguments.
   */
   ...
   * @param cb - callback function
   * 
   * @returns The response is a buffer 
   */
   ...
   * @param opts.type
   * @param opts.records
   * @param opts.caseSensitive
   * @param cb - (err, res) => {
   * @returns The response from the Lattice is a success or failure.
   */
   ...
   * @internal
   * 
   * @param enc_request_code - The code for the encrypted request. This is a single byte
   * @param payload - The payload to be encrypted
   * @returns The encrypted request.
   */
   ...
   * @param cb - The callback function that will be called when the request is complete.
   * @param retryCount - The number of times to retry the request if the device is busy.
  ...
  /**
  * Connect will call `StartPairingMode` on the device, which gives the user 60 seconds to finalize
  * the pairing.
  * - This will return an ephemeral public key, which is needed for the next request. 
  ...
  */
   ...
   * @returns A string. If the string is null, the wallet was successfully decrypted. If the string is
   ...
   * 
   * @param [LE=false] - Whether to return the public key in little endian format.
   ...
   * @param [skipErrors=false] - If true, errors will be skipped and the function will return an object
